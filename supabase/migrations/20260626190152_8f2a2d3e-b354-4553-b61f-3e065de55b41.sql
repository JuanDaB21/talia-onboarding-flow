...
BEGIN
  ...
  -- Buscar el pedido vigente: primero un ABIERTO; si no hay, el más reciente
  -- CONFIRMADO de la mesa (no pagado, no cerrado, no cancelado).
  ...
  LOOP
    -- Si el pedido reutilizado está CONFIRMADO, snapshot del destino y tiempo
    -- para que cocina/barra reciba los items nuevos como parte del mismo pedido.
    v_destino := NULL;
    v_tiempo := NULL;
