CREATE OR REPLACE FUNCTION public.registrar_pago(p_id_mesa uuid, p_metodo metodo_pago, p_subtipo text, p_voucher text, p_url_comprobante text, p_item_ids uuid[], p_propina numeric DEFAULT 0, p_id_bono uuid DEFAULT NULL::uuid, p_id_reserva uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO pagos (
    ...
    CASE WHEN p_id_reserva IS NOT NULL
      THEN trim(concat(NULLIF(trim(p_subtipo),''), ' Reserva ', v_codigo_reserva))
...
CREATE OR REPLACE FUNCTION public.registrar_pago_dividido(p_id_mesa uuid, p_item_ids uuid[], p_partes jsonb, p_propina numeric DEFAULT 0, p_id_bono uuid DEFAULT NULL::uuid, p_id_reserva uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF abs(v_suma_partes - v_total_requerido) > 1 THEN
    RAISE EXCEPTION 'La suma de partes (%) no coincide con el total requerido (%)', v_suma_partes, v_total_requerido USING ERRCODE='23514';
  END IF;
  ...
  v_subtipo_final := CASE WHEN p_id_reserva IS NOT NULL
    THEN trim(concat(v_subtipo, ' Reserva ', v_codigo_reserva))
    ELSE v_subtipo END;
